...
export async function resolveMainMethod(uri: string, token?: vscode.CancellationToken): Promise<IMainMethod[]> {
  if (token) {
    return <IMainMethod[]>(
...
export function resolveMainClass(workspaceUri?: string): Promise<IMainClassOption[]> {
  if (workspaceUri) {
...
export function resolveMainClassFromProject(projectName: string): Promise<IMainClassOption[]> {
  return <Promise<IMainClassOption[]>>(
    commands.executeJavaLanguageServerCommand(commands.JAVA_RESOLVE_MAINCLASS, projectName)
  )
}
...
export function inferLaunchCommandLength(config: any): Promise<number> {
